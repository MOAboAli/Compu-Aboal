class ServiceRequestController {
  ...
  cancel = async (req, res) => {
    try {
      res.json(await this.serviceRequestService.cancel(req.params.id, req.user));
    } catch (error) {
      res.status(error.statusCode || 400).json({ message: error.message });
    }
  };
  ...
  reschedule = async (req, res) => {
    try {
      res.json(
        await this.serviceRequestService.reschedule(req.params.id, req.user, req.body.preferredDate)
      );
    } catch (error) {
      res.status(error.statusCode || 400).json({ message: error.message });
    }
